...
點解有用：whisper 跟語言模型「順稿」，重讀 / 結巴位會被平滑化，淨睇文字會剪漏。
波形 valley（靜默＝凹位）係唔靠語言模型嘅 deterministic 信號 —— AI 睇住波形落刀，
交叉核 Gemini 報嘅秒數，兩者一致先剪。跨平台字體 fallback（Mac / Linux / Windows）。
...
# -------- Audio envelope (manual WAV RMS, no librosa dep) --------------------
...
# 粵語 word-label 要 CJK glyph，唔則中文字 render 成 tofu（□ 豆腐方格）。CJK 排第一，跨平台。
# Linux 要中文字體：apt install fonts-noto-cjk
CJK_FONT_CANDIDATES = [
    # macOS
...
def load_font(size: int) -> ImageFont.ImageFont:
    return _load_from(FONT_CANDIDATES, size)


def load_cjk_font(size: int) -> ImageFont.ImageFont:
    # CJK 字體擺第一，揾唔到先 fallback 去 mono（避免中文 tofu）
    return _load_from(CJK_FONT_CANDIDATES + FONT_CANDIDATES, size)
...
def render_timeline(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        small_font = load_cjk_font(13)  # word label — 粵語要 CJK 字體
        ...
        # Word labels above the waveform.
        # 注：reel-auto-cut transcript 嘅 word 冇 "type" field（純 {start,end,text}）。
        # 只有「有 type 而且唔係 word」先 skip；冇 type 當係 word（唔則全部字被 skip 冇 label）。
        last_label_x = -9999
        for w in words:
            if w.get("type") != "word" and "type" in w:
                continue
            ws = w.get("start")
            we = w.get("end")
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="Path to transcript.json for word labels + silence shading. "
             "If omitted, auto-resolves to <video_parent>/stt/transcript.json "
             "(reel-auto-cut work layout) or <video_parent>/transcript.json",
    )
    args = ap.parse_args()
    ...
    # Auto-resolve transcript if not given — try work layout then sibling
    transcript = args.transcript
    if transcript is None:
        for cand in (
            video.parent / "stt" / "transcript.json",
            video.parent / "transcript.json",
    ...
    if out_path is None:
        out_path = Path.cwd() / f"{video.stem}_{args.start:.2f}-{args.end:.2f}.png"
