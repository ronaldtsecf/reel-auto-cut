...
# 出:    proof.wav + proof_stt/ + proof_pass.json（綁 exact EDL/source/proof）
...
"$PY" "$SK/render_edl.py" "$WORK/edl.json" --out "$WORK/proof.wav" \
    --quality rough --audio-only 2>&1 | grep -E "silence-trim|final:" || true
...
SCAN_RC=0
if [ -f "$WORK/proof_allow.txt" ]; then
    "$PY" "$SK/repeat_scan.py" "$WORK/proof_stt/transcript.json" \
        --allow "$WORK/proof_allow.txt" || SCAN_RC=$?
else
    "$PY" "$SK/repeat_scan.py" "$WORK/proof_stt/transcript.json" || SCAN_RC=$?
fi
...
echo ""
if [ "$SCAN_RC" = "0" ]; then
    "$PY" "$SK/proof_receipt.py" record "$WORK"
    echo "═══ PROOF PASS — 開綠燈，可以 reel_finish render 真片 ═══"
else
    rm -f "$WORK/proof_pass.json"
